...
windows 安装： https://www.jianshu.com/p/414ccd423efc

```
ssh-keygen -t rsa -C maofengqi@hpcang.com
id_rsa_gitlab
git config --global user.name maofengqi
git config --global user.email maofengqi@hpcang.com

ssh-keygen -t rsa -C fengqi.mao.march@gmail.com
id_rsa_github
...
```
# gitlab.hpcang.com
Host gitlab.hpcang.com
    HostName gitlab.hpcang.com
...
# maofengqi@hpcang.com
function gitlab(){
    git config --global user.email maofengqi@hpcang.com
    git config --global user.name maofengqi
    echo "maofengqi@hpcang.com"
}
...
github(){
    git config --global user.email fengqi.mao.march@gmail.com
    git config --global user.name maoqiqi
    echo "Github Name--`git config --global user.name`,Email--`git config --global user.email`"
}
```

账号切换查看:`cat ~/.gitconfig`
...
# verystar
Host git.verystar.cn
    HostName git.verystar.cn
    IdentityFile ~/.ssh/id_rsa_verystar
    PreferredAuthentications publickey
```
...
```
proxy(){
    export http_proxy=http://10.64.146.3:8123
    export https_proxy=http://10.64.146.3:8123
    echo "proxy"
}
...
# tools
export PATH=/Users/march/tools:$PATH
...
MarkdownPad 报错：An error occurred with the Html rendering component

https://blog.csdn.net/gxseveryday/article/details/80031755

https://www.jianshu.com/p/168ead26c751
